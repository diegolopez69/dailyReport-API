...
exports.projectors = async (req, res) => {
  try {
    // Query to get the actual count of projectors that physically exist
    const getAmountOfProjectorsIfPhysicallyExist = await db.sequelize.query(
    ...
    const getTheoreticalAmountOfKeyboards = await db.sequelize.query(
      "SELECT months.month AS month, COALESCE(t.quantity_proyectores, 0) AS quantity_proyectores FROM ( SELECT 1 AS month UNION SELECT 2 UNION SELECT 3 UNION SELECT 4 UNION SELECT 5 UNION SELECT 6 UNION SELECT 7 UNION SELECT 8 UNION SELECT 9 UNION SELECT 10 UNION SELECT 11 UNION SELECT 12 ) AS months LEFT JOIN ( SELECT MONTH(createdAt) AS month, COUNT(*) AS quantity_proyectores FROM tb_inventories WHERE Tool_id = 11 AND YEAR(createdAt) = YEAR(CURRENT_DATE()) GROUP BY MONTH(createdAt) ) AS t ON months.month = t.month ORDER BY months.month;",
      { type: db.sequelize.QueryTypes.SELECT }
    );
